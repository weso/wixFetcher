...
class Parser(object):
    ...
    def _parse_country_data(self, sheet, row, country_name, sheet_name):
        ...
        while i < sheet.ncols - 2:
            ...
            if obs_value in [None, "", " "]:
                break
            year_value = int(sheet.row(Parser.YEAR_ROW)[i].value)
    ...
    def _add_propper_computation_to_observation(self, observation, computation_type):
        if computation_type == "raw":
            observation.add_computation(type="raw")
        if computation_type == "ranked":
            observation.add_computation(type="ranked",
                                        reason="",  # TODO: ??
                                        slice=None,
                                        dimension=None)  # TODO: probably not None
        if computation_type == "normalized":
        ...
        if computation_type == "grouped":
            observation.add_computation(type="grouped")
            # ... )
            #TODO. As i see in the old version through wesby, there are
            pass
